...
Caching is turned on by default when a `packageManager` or `dev.engines.packageManager` field is detected in the `package.json` file. The `default-cache` input provides control over this automatic caching behavior. By default, `default-cache` is set to `true`, which enables caching when a valid package manager field is detected in the `package.json` file. To disable this automatic caching, set the `default-cache` input to `false`.
